...
(defn- start!
  ...
  [{:keys [^EventLoopGroup boss ^EventLoopGroup worker socket-class ^SslContext ssl-context config]} protocol port use-ssl?]
  (when (and (number? port) (> (int port) 0))
    (if (and use-ssl? (nil? ssl-context))
      ...
      (let [initializer (ch-init/new-channel-initializer {:use-ssl? use-ssl?
                                     ...
                                     :messages-dispatcher-fn #(md/new-messages-dispatcher)})
...
(defn start
  ...
  ([config ^SslContext ssl-context]
   (let [boss (NioEventLoopGroup.)
         ...
         base {:boss boss :worker worker :socket-class socket-class :ssl-context ssl-context :config config}
