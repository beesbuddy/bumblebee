(defproject bumblebee "0.1.0-SNAPSHOT"
  ...
  :javac-options ["-source" "21" "-target" "21"]
  :jar-exclusions    [#"^java.*"]
  ;; No AOT needed for Clojure record-based Subscription
  ...
  :aliases {:run {:main-opts ["-m" "bumblebee.main"]}
            ...
            ["shell"
             ...
             "--initialize-at-run-time=org.sqlite.JDBC,org.httpkit.client.ClientSslEngineFactory\\$SSLHolder"
             "--enable-url-protocols=http,https,jdbc,jar"
             "--enable-all-security-services"
             "-jar" "./target/${:uberjar-name:-${:name}-${:version}-standalone.jar}"
             "-Dorg.sqlite.lib.exportPath=~/native"
             "-H:Name=./target/${:name}"
