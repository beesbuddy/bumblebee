(ns user
  (:require
   ...
   [dev.filters-demo :as fdemo])
  (:import
   [io.netty.handler.ssl SslContextBuilder]
   [io.netty.handler.ssl.util SelfSignedCertificate InsecureTrustManagerFactory]
   [io.netty.bootstrap Bootstrap]
   [io.netty.channel.nio NioEventLoopGroup]
   [io.netty.channel.socket.nio NioSocketChannel]
   [io.netty.channel ChannelInitializer ChannelHandler ChannelPipeline Channel SimpleChannelInboundHandler]
   [io.netty.handler.codec.mqtt MqttConnAckMessage MqttConnAckVariableHeader MqttMessage MqttDecoder MqttEncoder MqttMessageBuilders MqttVersion]
...
(defn stop []
  ;; http-kit exposes a global stop; adjust if you keep a server ref instead.
  (try
    (http-kit/server-stop!)
    (catch Throwable _e nil)))

(defn reset []
  (stop)
  ;; Limit refresh to project source dirs
...
(defn deny-all-on!
  ([] (deny-all-on! 0))
  ([priority]
   (fx/enable-deny-all! priority)))

(defn deny-all-off! []
  (fx/remove! ::fx/deny-all))
